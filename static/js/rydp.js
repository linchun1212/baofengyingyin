ajax({
    url:'static/js/rydp.json',
    success:function(result){
        rydp1(JSON.parse(result))
        // ltj(JSON.parse(result))
    }
})
function rydp1(result){
    var rydp = document.getElementById('rydp')
    var recoUl = rydp.getElementsByTagName('ul')[0]
    var xin=result
    var str=''
    var len=xin.length
    console.log(len)
